        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="rounded-xl border border-q-border bg-q-bg p-5">
              ...
              <div className="mt-2 text-3xl font-bold text-q-text">{data.counts.implementedRequests}</div>
            </div>
          </div>
        ) : null}
      </SectionCard>
                  ...
                  <div key={item.month}>
                    <div className="mb-2 flex items-center justify-between text-sm">
                      ...
                      <span className="text-q-muted">
                        Total {item.total} · Tools {item.tools} · Calculators {item.calculators} · AI {item.aiTools}
                      </span>
                    </div>

                    <div className="h-4 rounded-full bg-q-bg">
                      <div
                        className="h-4 rounded-full bg-q-primary"
                        style={{ width }}
                      />
                    </div>
                  </div>
              ...
              {data.enginePerformance.map((item) => (
                <div key={item.engine_type} className="rounded-xl border border-q-border bg-q-bg p-4">
                  ...
                  <div className="mt-2 grid gap-2 text-sm text-q-muted sm:grid-cols-2">
                    <div>Tools: <span className="font-medium text-q-text">{item.tool_count}</span></div>
                    <div>Total usage: <span className="font-medium text-q-text">{item.total_usage}</span></div>
                    <div>Avg/tool: <span className="font-medium text-q-text">{item.avg_usage_per_tool}</span></div>
                    <div>Top slug: <span className="font-medium text-q-text">{item.top_tool_slug || "-"}</span></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid gap-4">
              {data.nichePerformance.map((item) => (
                <div key={item.key} className="rounded-xl border border-q-border bg-q-bg p-4">
                  ...
                  <div className="mt-2 grid gap-2 text-sm text-q-muted sm:grid-cols-2">
                    <div>Live tools: <span className="font-medium text-q-text">{item.tool_count}</span></div>
                    <div>Total usage: <span className="font-medium text-q-text">{item.total_usage}</span></div>
                  </div>
                </div>
              ))}
              ...
              {data.demandGaps.map((item) => (
                <div key={item.niche_key} className="rounded-xl border border-q-border bg-q-bg p-4">
                  ...
                  <div className="mt-3 grid gap-2 text-sm text-q-muted sm:grid-cols-2">
                    <div>Request mentions: <span className="font-medium text-q-text">{item.request_mentions}</span></div>
                    <div>Live tools: <span className="font-medium text-q-text">{item.live_tool_count}</span></div>
                  </div>
