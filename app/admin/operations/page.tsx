...
export default function AdminOperationsPage() {
  ...
  async function loadAll() {
    ...
    try {
      const [recentRes, fallbackRes, repairPreviewRes] = await Promise.all([
        fetchJson("/api/admin/recent-items"),
        fetchJson("/api/admin/fallback-audit"),
        fetchJson("/api/admin/repair-preview"),
      ]);
      ...
      setError(err instanceof Error ? err.message : "Failed to load admin operations.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function runEnginePreview() {
    ...
    try {
      const response = await fetch("/api/admin/engine-preview", {
        method: "POST",
        headers: {
  ...
  async function runBulkPreview() {
    ...
    try {
      const response = await fetch("/api/admin/bulk-preview", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function executeRepair() {
    ...
    try {
      const response = await fetch("/api/admin/execute-repair", {
        method: "POST",
      });
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <div className="grid gap-2">
                      {fallback.calculatorsMissingRuntime.slice(0, 12).map((item) => (
                        <div
                  ...
                  <div>
                    ...
                    <div className="grid gap-2">
                      {fallback.formulaCalculatorCandidates.slice(0, 12).map((item) => (
                        <div
