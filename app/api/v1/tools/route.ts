...
export async function GET(req: Request) {
  ...
  try {
    ...
    let results = (data || []).map(t => ({
      ...
      type: "tool",
      url: `https://quickfnd.com/tools/${t.slug}`,
      embed_url: `https://quickfnd.com/embed/tools/${t.slug}`,
    }));
    ...
    return NextResponse.json({ success: false, error: "Failed to fetch tools" }, { status: 500, headers: CORS });
  }
}