    ...
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={generate} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            {mode === "password" ? "Generate Password" : mode === "uuid" ? "Generate UUID" : "Generate String"}
          </button>
    ...
    <Card title={title}>
      <div className="space-y-4">
        <textarea
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={mode === "json" ? 'Paste JSON here, for example: {"name":"QuickFnd"}' : "Paste code or JSON here"}
          className={textareaClass("min-h-[180px]")}
        />

        <div className="flex flex-wrap gap-3">
          <button onClick={() => formatValue(true)} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            Format
          </button>
          {allowMinify ? (
            <button onClick={() => formatValue(false)} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover">
              Minify
            </button>
          ) : null}
  ...
  return (
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={runCodec} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            Run
          </button>
          <button onClick={() => copyText(output)} disabled={!output} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
            Copy
...
function RegexTools({
  ...
  function runRegex() {
    try {
      ...
      const list = Array.from(sourceText.matchAll(regex)).map((match) => match[0]);
      setMatches(list);
      setSummary(regex.test(sourceText) ? `Pattern matched. Total matches: ${list.length}.` : "No matches found.");
      setError("");
    } catch {
      setMatches([]);
      setSummary("");
  ...
  return (
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={runRegex} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            {mode === "extract" ? "Extract Matches" : "Test Regex"}
          </button>
          <button onClick={() => copyText(matches.join("\n"))} disabled={matches.length === 0} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
            Copy Matches
          </button>
        </div>
  ...
  return (
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={generateHash} disabled={working} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover disabled:opacity-60">
      ...
      <div className="space-y-6">
        <div className={softPanelClass()}>
          ...
          <div className="space-y-3">
            ...
            />
            <div className="flex flex-wrap gap-3">
              <button onClick={fromUnix} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
                Convert To Date
              </button>
              <button onClick={() => copyText(dateOutput)} disabled={!dateOutput} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
                Copy Date
              </button>
            </div>
            <textarea
        ...
        <div className={softPanelClass()}>
          ...
          <div className="space-y-3">
            ...
            />
            <div className="flex flex-wrap gap-3">
              <button onClick={toUnix} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
                Convert To Unix
              </button>
              <button onClick={useNow} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover">
                Use Current Time
              </button>
              <button onClick={() => copyText(unixOutput)} disabled={!unixOutput} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
                Copy Unix
              </button>
            </div>
            <textarea
...
function ColorTools({
  ...
  function convertColor() {
    try {
      ...
      setOutput("");
      setPreviewColor("");
      setError(mode === "hex-to-rgb" ? "Invalid hex color." : "Invalid RGB input. Use format like 255, 99, 71");
    }
  }
  ...
  return (
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={convertColor} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            Convert Color
          </button>
          <button onClick={() => copyText(output)} disabled={!output} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
            Copy Output
        ...
        {previewColor ? (
          <div className={softPanelClass()}>
            ...
            <div className="h-20 rounded-xl border border-q-border" style={{ backgroundColor: previewColor }} />
          </div>
        ) : null}
  ...
  return (
    <Card title={title}>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={convert} className="rounded-xl bg-q-primary px-4 py-2 font-medium text-white hover:bg-q-primary-hover">
            Convert
          </button>
          <button onClick={() => copyText(output)} disabled={!output} className="rounded-xl border border-q-border bg-q-card px-4 py-2 font-medium text-q-text hover:bg-q-card-hover disabled:opacity-50">
            Copy Output
...
export default function BuiltInToolClient({ item }: Props) {
  ...
  if (preset.family === "currency-converter") {
    return <CurrencyConverterClient />;
  }

  if (preset.family === "strength-checker") {
    return <StrengthChecker title={preset.title} config={preset.config} />;
  }

  if (preset.family === "string-generator") {
    return <StringGenerator title={preset.title} config={preset.config} />;
  }

  if (preset.family === "text-formatter") {
    return <TextFormatter title={preset.title} config={preset.config} />;
  }

  if (preset.family === "text-analyzer") {
    return <TextAnalyzer title={preset.title} config={preset.config} />;
  }

  if (preset.family === "text-transformer") {
    return <TextTransformer title={preset.title} config={preset.config} />;
  }

  if (preset.family === "codec") {
    return <CodecTool title={preset.title} config={preset.config} />;
  }

  if (preset.family === "snippet-manager") {
    return <SnippetManager title={preset.title} />;
  }

  if (preset.family === "number-generator") {
    return <NumberGenerator title={preset.title} config={preset.config} />;
  }

  if (preset.family === "unit-converter") {
    return <UnitConverter title={preset.title} config={preset.config} />;
  }

  if (preset.family === "regex-tools") {
    return <RegexTools title={preset.title} config={preset.config} />;
  }

  if (preset.family === "hash-tools") {
    return <HashTools title={preset.title} config={preset.config} />;
  }

  if (preset.family === "timestamp-tools") {
    return <TimestampTools title={preset.title} />;
  }

  if (preset.family === "color-tools") {
    return <ColorTools title={preset.title} config={preset.config} />;
  }

  if (preset.family === "developer-converters") {
    return <DeveloperConverters title={preset.title} config={preset.config} />;
  }

  return <GenericTool title={preset.title} description={preset.description} />;
}