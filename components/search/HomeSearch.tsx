      ...
      <div className="mt-4 space-y-3">
        {items.map((item) => (
          <Link
            ...
            href={item.href}
            className="block rounded-xl border border-q-border bg-q-bg p-4 transition hover:bg-q-card-hover"
          >
            <div className="flex items-center justify-between gap-3">
              <h4 className="font-medium text-q-text">{item.name}</h4>
              <span className="rounded-full border border-q-border px-2 py-1 text-xs text-q-muted">
                {item.type}
              </span>
            </div>
