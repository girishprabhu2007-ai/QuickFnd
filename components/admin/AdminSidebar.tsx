...
const GROUPS: MenuGroup[] = [
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <rect x="1.5" y="1.5" width="7" height="7" rx="2" fill="currentColor" opacity="0.85" />
        <rect x="11.5" y="1.5" width="7" height="7" rx="2" fill="currentColor" opacity="0.35" />
        <rect x="1.5" y="11.5" width="7" height="7" rx="2" fill="currentColor" opacity="0.35" />
        <rect x="11.5" y="11.5" width="7" height="7" rx="2" fill="currentColor" opacity="0.35" />
      </svg>
    ),
    items: [
  ...
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <line x1="6" y1="14" x2="9.5" y2="14" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
      </svg>
    ),
    items: [
  ...
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <path d="M3 13.5v2.5a2 2 0 002 2h10a2 2 0 002-2v-2.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
    items: [
  ...
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <polyline points="2.5,16 7,9 11,11.5 17.5,4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        <circle cx="17.5" cy="4" r="2" fill="currentColor" />
      </svg>
    ),
    items: [
  ...
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <text x="10" y="14" textAnchor="middle" fill="currentColor" fontSize="11" fontWeight="500">$</text>
      </svg>
    ),
    items: [
  ...
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <path d="M10 1.5v2.5M10 16v2.5M1.5 10H4M16 10h2.5M4.2 4.2l1.8 1.8M14 14l1.8 1.8M15.8 4.2l-1.8 1.8M6 14l-1.8 1.8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
      </svg>
    ),
    items: [
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex w-[56px] flex-col items-center gap-1.5 bg-[#16131f] py-3 dark:bg-[#0d0b14]">
        ...
        {/* Logo */}
        <Link href="/admin" className="mb-4 flex h-9 w-9 items-center justify-center rounded-xl text-sm font-black text-white" style={{ background: "var(--q-gradient-purple)" }}>
          Q
        </Link>
            ...
            <button
              ...
              className={`relative flex h-10 w-10 items-center justify-center rounded-xl transition-colors ${
                isActive
                  ...
                  : "text-white/[0.35] hover:bg-white/[0.07] hover:text-white/[0.65]"
              }`}
            >
              {isActive && (
                <span className="absolute left-0 top-2 h-6 w-[3px] rounded-r-full bg-[#7c3aed]" />
              )}
              {group.icon}
              {hasAlert && (
                <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-red-500 ring-2 ring-[#16131f]" />
              )}
            </button>
          );
        })}
      ...
      {!collapsed && (
        <div className="flex w-[210px] flex-col border-r border-q-border bg-q-bg overflow-y-auto">
          ...
          <div className="px-4 pb-1 pt-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-q-muted">
              {currentGroup.label}
            </p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  {item.badge === "count" && badgeValue !== undefined && badgeValue > 0 && (
                    <span className="rounded-md bg-q-card px-1.5 py-0.5 text-[11px] tabular-nums text-q-muted border border-q-border">
                      {badgeValue}
                    </span>
                  )}
                  {item.badge === "alert" && badgeValue !== undefined && badgeValue > 0 && (
                    <span className="rounded-md bg-amber-100 px-1.5 py-0.5 text-[11px] font-bold tabular-nums text-amber-700 dark:bg-amber-500/15 dark:text-amber-400">
                      {badgeValue}
                    </span>
                  )}
                </Link>
          ...
          {/* Divider + other groups quick-switch */}
            ...
            <div className="flex flex-wrap gap-1">
              {GROUPS.filter((g) => g.key !== activeGroup).map((group) => (
                <button
                  ...
                  className="rounded-md px-2 py-1 text-[11px] text-q-muted transition-colors hover:bg-q-card-hover hover:text-q-text"
                  title={group.label}
                >
                  {group.label}
                </button>
